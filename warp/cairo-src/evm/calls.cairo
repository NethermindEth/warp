...
func uint256_to_address_felt{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(
        address_big : Uint256) -> (res : felt):
    alloc_locals
...
func warp_call{
    ...
    let (local success, local returndata_size, local returndata_len,
        local returndata) = GenericCallInterface.fun_ENTRY_POINT(
...
func returndata_write{memory_dict : DictAccess*, exec_env : ExecutionEnvironment*, range_check_ptr}(
    ...
    alloc_locals
    let (local __fp__, _) = get_fp_and_pc()
    let (local returndata : felt*) = array_create_from_memory(
        returndata_ptr.low, returndata_size.low)
    local memory_dict : DictAccess* = memory_dict
