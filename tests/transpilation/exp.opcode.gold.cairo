...
from starkware.cairo.common.uint256 import Uint256, uint256_eq, uint256_exp
...
func segment0{range_check_ptr, msize, memory_dict : DictAccess*}(stack : StackItem*) -> (
    ...
    let (local tmp0) = uint256_exp(Uint256(2, 0), Uint256(2, 0))
    let (local tmp1) = uint256_exp(Uint256(2, 0), tmp0)
    local newitem0 : StackItem = StackItem(value=tmp1, next=stack0)
