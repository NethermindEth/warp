...
async def test_address():
    ...
    contract_address = await deploy_contract(
        starknet, program_info, contract_definition, 500
    )

    res = await invoke_method(starknet, program_info, contract_address, "MY_UINT")
    assert res.retdata == [32, 2, 0, 500]

    res = await invoke_method(starknet, program_info, contract_address, "MY_ADDRESS")
