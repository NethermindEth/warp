...
import { deployContract, getSafeWithOwners } from './setup';
import {
  safeApproveHash,
  buildSignatureBytes,
...
describe('GnosisSafe', async () => {
  ...
  describe('execTransaction', async () => {
    ...
    it('should emit event for failed call execution if safeTxGas > 0', async () => {
      ...
      await expect(
        executeContractCallWithSigners(safe, reverter, 'revert', [], [user1], false, {
          safeTxGas: 1,
        }),
    ...
    it('should emit event for failed delegatecall execution  if safeTxGas > 0', async () => {
      ...
      const txHash = calculateSafeTransactionHash(
        safe,
        buildContractCall(reverter, 'revert', [], await safe.nonce(), true, {
          safeTxGas: 1,
        }),
        await chainId(),
      );
    ...
    it('should revert on unknown operation', async () => {
      ...
      await expect(executeTx(safe, tx, [await safeApproveHash(user1, safe, tx, true)])).to
        .be.reverted;
    });
    ...
    it('should emit payment in success event', async () => {
      ...
      await expect(await hre.ethers.provider.getBalance(safe.address)).to.be.deep.eq(
        parseEther('1'),
      );
      ...
      expect(successEvent.txHash).to.be.eq(
      ...
      await expect(await hre.ethers.provider.getBalance(user2.address)).to.be.deep.eq(
        userBalance.add(successEvent.payment),
      );
    });
