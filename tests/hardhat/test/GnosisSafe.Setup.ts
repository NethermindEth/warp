...
// import { deployContract, getMock, getSafeSingleton, getSafeTemplate } from './setup';
// import { calculateSafeDomainSeparator } from '../utils/execution';
// import { AddressOne } from '../utils/constants';
// import { chainId, encodeTransfer } from './encoding';
...
//       await expect(await singleton.getModulesPaginated(AddressOne, 10)).to.be.deep.eq([
//         [],
//         AddressZero,
//       ]);
...
//           AddressZero,
//         );
//       await expect(await template.domainSeparator()).to.be.eq(
//         calculateSafeDomainSeparator(template, await chainId()),
//       );
//       await expect(await template.getOwners()).to.be.deep.eq([
//         user1.address,
//         user2.address,
//         user3.address,
//       ]);
...
//       const initData = testIntializer.interface.encodeFunctionData('init', [
//         '0x42baddad',
//       ]);
//       await expect(
//         template.setup(
...
//       await expect(await hre.ethers.provider.getBalance(template.address)).to.be.deep.eq(
//         parseEther('10'),
//       );
...
//         parseEther('0'),
//       );
//       await expect(userBalance.lt(await hre.ethers.provider.getBalance(user1.address))).to
//         .be.true;
//     });
...
//         parseEther('0'),
//       );
//       await expect(await hre.ethers.provider.getBalance(user2.address)).to.be.deep.eq(
//         userBalance.add(payment),
//       );
...
//       expect(
//         await mock.callStatic.invocationCountForCalldata(transferData),
