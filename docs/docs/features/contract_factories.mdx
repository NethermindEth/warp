...
Warp also supports the use of the salt option e.g `new Contract{salt: salt}()`, but please note that because of the differences
in Ethereum and StarkNet the salt will be truncated from 32 bytes to 30 bytes.
