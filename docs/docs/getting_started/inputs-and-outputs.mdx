...
Warp takes these inputs and transcodes them into Cairo to forwards them onto the desired network.
...
- Data types that are more than 248 bits wide (`int256`/`uint256`/`bytes32`) will need to be split into their upper and
  low 128 bits e.g `0x10000000000000000000000000000000f` as a `uint256` -> `15,1`.
...
| uint256 (below 128 bits)               |                 10                  |                                 0,10                                  |
