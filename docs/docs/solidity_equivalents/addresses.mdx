...
First, the bounds of an address are not checked at compile time, which can introduce strange behaviour. The expression `address(uint256(MAX_UINT256))` will compile even though the maximum value for addresses is `2**251 - 1`.
