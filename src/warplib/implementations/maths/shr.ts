...
export function shr(): void {
  generateFile(
    ...
    [
      ...
      'from starkware.cairo.common.uint256 import u256, uint256_and',
      'from warplib.maths.pow2 import pow2',
    ],
    forAllWidths((width) => {
      if (width === 256) {
        return [
          `func warp_shr256{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(lhs : u256, rhs : felt) -> (`,
          `        result : u256){`,
          ...
          `    return (u256(0, 0),);`,
          `}`,
          `func warp_shr256_256{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(lhs : u256, rhs : u256) -> (`,
          `        result : u256){`,
          `    if (rhs.high != 0){`,
          `        return (u256(0, 0),);`,
          `    }`,
...
export function shr_signed(): void {
  generateFile(
    ...
    forAllWidths((width) => {
      if (width === 256) {
        return [
          `func warp_shr_signed256{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(lhs : u256, rhs : felt) -> (res : u256){`,
          `    `,
          ...
          `                return (u256(logical_shift.low+ones*shift, ${mask(128)}),);`,
          `            }else{`,
          `                let (bound) = pow2(rhs);`,
          `                let ones = bound - 1;`,
          `                let (shift) = pow2(128-rhs);`,
          `                return (u256(logical_shift.low, logical_shift.high+ones*shift),);`,
          `            }`,
          `        }`,
          `    }`,
          `}`,
          `func warp_shr_signed256_256{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(lhs : u256, rhs : u256) -> (res : u256){`,
          `    if (rhs.high == 0){`,
