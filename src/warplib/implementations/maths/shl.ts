...
export function shl(): void {
  ...
  generateFile(
    ...
    [
      ...
      'from starkware.cairo.common.uint256 import u256, uint256_shl',
      'from warplib.maths.pow2 import pow2',
    ],
    forAllWidths((width) => {
      if (width === 256) {
        return [
          'func warp_shl256{range_check_ptr}(lhs : u256, rhs : felt) -> (result : u256){',
          '    let (high, low) = split_felt(rhs);',
          '    let (res) = uint256_shl(lhs, u256(low, high));',
          '    return (res,);',
          '}',
          'func warp_shl256_256{range_check_ptr}(lhs : u256, rhs : u256) -> (result : u256){',
