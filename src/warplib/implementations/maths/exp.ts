...
function createExp(signed: boolean, unsafe: boolean) {
  ...
  generateFile(
    ...
    forAllWidths((width) => {
      if (width === 256) {
        return [
          `func _repeated_multiplication${width}{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(op : u256, count : felt) -> (res : u256){`,
          `    if (count == 0){`,
          `        return (u256(1, 0),);`,
          `    }`,
          ...
          `    return (res,);`,
          `}`,
          `func warp_exp${suffix}${width}{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(lhs : u256, rhs : felt) -> (res : u256){`,
          `    if (rhs == 0){`,
          `        return (u256(1, 0),);`,
          '    }',
          '    if (lhs.high == 0){',
          ...
          `    return (res,);`,
          `}`,
          `func _repeated_multiplication_256_${width}{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(op : u256, count : u256) -> (res : u256){`,
          `    if (count.low == 0 and count.high == 0){`,
          `        return (u256(1, 0),);`,
          `    }`,
          `    let (decr) = uint256_sub(count, u256(1, 0));`,
          `    let (x) = _repeated_multiplication_256_${width}(op, decr);`,
          `    let (res) = warp_mul${suffix}${width}(op, x);`,
          `    return (res,);`,
          `}`,
          `func warp_exp_wide${suffix}${width}{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(lhs : u256, rhs : u256) -> (res : u256){`,
        ...
        return [
          ...
          `    return (res,);`,
          '}',
          `func _repeated_multiplication_256_${width}{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(op : felt, count : u256) -> (res : felt){`,
          `    `,
          `    if (count.low == 0 and count.high == 0){`,
          `        return (1,);`,
          `    }`,
          ...
          `    return (res,);`,
          `}`,
          `func warp_exp_wide${suffix}${width}{range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(lhs : felt, rhs : u256) -> (res : felt){`,
          '    if (rhs.low == 0){',
          '        if (rhs.high == 0){',
          '            return (1,);',
          '        }',
...
function getNegativeOneShortcutCode(signed: boolean, lhsWidth: number, rhsWide: boolean): string[] {
  ...
  if (lhsWidth < 256) {
    ...
    return [
      ...
      `    let (is_odd) = bitwise_and(${rhsWide ? 'rhs.low' : 'rhs'}, 1);`,
      `    return (u256(1 + is_odd * 0x${'f'.repeat(31)}e, is_odd * ${mask(128)}),);`,
      `}`,
    ];
  }
}
