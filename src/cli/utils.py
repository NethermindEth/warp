...
def get_offsets(language):
    if language == ContractLang.VYPER:
        jumpdest_pos_offset = 6
        increment = 8
    elif language == ContractLang.SOL:
        jumpdest_pos_offset = 4
        increment = 7
    return increment, jumpdest_pos_offset
...
def selector_jumpdest(
    ...
    language = get_language(opcodes_path)
    increment,jumpdest_pos_offset = get_offsets(language)
    ...
    idx = 0
    while idx < l:
        if is_entry_seq(all_ops[idx : idx + increment + 1], language):
            try:
                selector = all_ops[idx + 1].lower()
                func_sig = selectors[selector]
                selector_jumpdests[selector] = {
                    func_sig: int(all_ops[idx + jumpdest_pos_offset], 16)
                }
                idx += inc
                continue
            except KeyError:
