...
export class CairoContractWriter extends CairoASTNodeWriter {
  writeInner(node: CairoContract, writer: ASTWriter): SrcDesc {
    ...
    const imports = writeImports(this.ast.getImports(sourceUnit))
      .split('\n')
      .map((l) => (l.length > 0 ? INDENT + l : l))
      .join('\n');
    ...
    const freeStructs = node ? getStructs(sourceUnit, this.ast) : [];
    const structs = [...freeStructs, ...node.vStructs, ...(node.vStructs || [])]
      .map((v) => writer.write(v))
      .join('\n');

    const fileLevelFunctions = sourceUnit.vFunctions.map((v) => writer.write(v)).join('\n');
